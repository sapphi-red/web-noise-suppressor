import { SpeexPreprocessor, type SpeexModule } from '@sapphi-red/speex-preprocess-wasm'
...
export const createProcessor = (
  ...
  const getProperty = <K extends keyof SpeexPreprocessor>(k: K): SpeexPreprocessor[K] => {
    return preprocessors[0]![k]
  }
  const setProperty = <K extends keyof SpeexPreprocessor>(k: K, v: SpeexPreprocessor[K]) => {
    for (const preprocessor of preprocessors) {
      preprocessor[k] = v
    }
  }
